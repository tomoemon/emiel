...
const s = (key: VirtualKey) => new SingleStroke(key, AndModifier.empty);
...
function inputKeys(entry: RuleEntry): string[] {
  return entry.input.map((i) => (i as SingleStroke).key);
}

function nextInputKeys(entry: RuleEntry): string[] {
  return entry.nextInput.map((i) => (i as SingleStroke).key);
}
