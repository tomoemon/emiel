...
export type KeyEventType = "keyup" | "keydown"; export class InputStroke {
...
export function matchCandidateEdge(event: InputEvent, edge: StrokeEdge): MatchResult {
	...
	if (event.input.key === edge.input.key) {
		...
		if (edge.input.requiredModifier.accept(event.keyboardState)) {
			return { type: "matched", keyCount: edge.input.requiredModifier.groups.length + 1 };
		}
...
export function matchOtherEdge(event: InputEvent, rule: Rule, candidateEdges: StrokeEdge[]): MatchResult {
	...
	if (otherMatched.length > 0) {
		const keyCount = Math.max(...otherMatched.map((e) => e.input[0].requiredModifier.groups.length)) + 1;
