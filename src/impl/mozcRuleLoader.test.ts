...
test("load google ime single rule", () => {
  ...
  expect((entries[0].input[0] as SingleStroke).key).toEqual(VirtualKeys.A);
...
test("load google ime next rule", () => {
  ...
  expect((entries[0].input[0] as SingleStroke).key).toEqual(VirtualKeys.T);
  expect((entries[0].input[1] as SingleStroke).key).toEqual(VirtualKeys.T);
  expect(entries[0].output).toEqual("っ");
  expect(entries[0].nextInput.length).toEqual(1);
  expect((entries[0].nextInput[0] as SingleStroke).key).toEqual(VirtualKeys.T);
});
...
`,
  ...
  expect(entries[0].output).toEqual("あ");
  expect(entries[0].nextInput).toEqual([]);
  expect((entries[1].input[0] as SingleStroke).key).toEqual(VirtualKeys.I);
