...
const physicalKeyboardLayouts: PhysicalKeyboardLayout[] = [
	 ...
	 */
  {
    ...
    keys: [
      ...
      [VirtualKeys.LangLeft, VirtualKeys.Space, VirtualKeys.LangRight],
    ],
...
class Rect {
  constructor(
    readonly leftTop: Coord2d,
    readonly rightBottom: Coord2d,
  ) {}
  get width(): number {
...
export class KeyboardGuide {
  ...
  constructor(readonly guideData: KeyboardGuideLabelMapping) {
    const physicalLayout = physicalKeyboardLayouts.filter(
      (v) => v.name === guideData.physicalLayout,
    );
  ...
  keyTops(
    ...
    const positions = arrangeKeyPositions(
      this.physicalLayout,
      size.keyWidth,
      size.keyHeight,
      size.gapX,
...
function arrangeKeyPositions(
  ...
  keys.forEach((row, i) => {
    ...
    row.forEach((key) => {
      const x = leftMargin;
      const width = keyWidth[key.toString()]
        ? keyWidthUnit * keyWidth[key.toString()]
        : keyWidthUnit;
      leftMargin += width + keyGapX;
      const y = i * (keyHeightUnit + keyGapY);
      keyPositions.push({
        key,
        rect: new Rect({ x: x, y: y }, { x: x + width, y: y + keyHeightUnit }),
      });
    });
  });
  return keyPositions;
