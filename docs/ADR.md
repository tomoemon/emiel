...
# 20260409 Automaton API リデザイン: build + ビルダーチェーン

## 背景

Automaton の状態参照方法に一貫性がなく、ユーザーにとって分かりづらかった。

- `automaton.xxx()` と `AutomatonGetters.xxx(automaton)` の2つの参照パターン
- ローマ字専用クエリ（getFinishedRoman）と共通クエリの混在
- backspace 対応クエリ（getEffectiveEdges）と非対応クエリの混在
- `rule.build()` が Rule の責務を超えている

## 決定事項

- `rule.build()` を廃止し、スタンドアロン関数 `build(rule, kanaText)` を導入
- クエリを3グループに分割: 基本 / ローマ字拡張 / backspace 拡張
- ビルダーチェーン `.withRoman()` / `.withBackspace()` で必要な機能を追加
- `AutomatonGetters`（現 `automatonQuery`）は公開 API から削除し内部モジュールとする
- `with()` によるカスタム拡張は維持

## 検討した代替案

- クエリの分類を Automaton のサブクラスで表現する案: 組み合わせ爆発（ローマ字×backspace 等）が起きるため却下
- 既存クエリを withBackspace 時に上書きする案: ユーザーが「同じメソッド名で挙動が変わる」ことを意識する必要があり混乱を招くため、別名で追加する方針を採用
- `rule.build()` を便利メソッドとして残す案: 責務の明確化を優先し廃止。build が import で直接見つかるため発見性の問題は小さい

## 詳細

[automaton-api-design.md](./automaton-api-design.md) を参照。
