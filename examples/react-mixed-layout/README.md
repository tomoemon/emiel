...
emiel の preset は「素の Rule」のみを返すので、consumer が `createDirectInputRule(layout)` を `merge` するだけで混在構成が作れる。

```ts
const rule = loadPresetRuleRoman(kanaLayout).merge(createDirectInputRule(alphaLayout));
```
